...
uhal_status_t spi_host_init(const spi_host_inst_t spi_peripheral_num, const uint32_t spi_clock_source, const uint32_t spi_clock_source_freq,
    ...
    const uint8_t databits = (BITMASK_COMPARE(spi_extra_configuration_opt, SPI_BUS_OPT_CHAR_SIZE_10_BIT) >> 2) + 3;
    const uint8_t msbfirst = ~((BITMASK_COMPARE(spi_extra_configuration_opt, SPI_BUS_OPT_DATA_ORDER_LSB_FIRST) >> 1));
